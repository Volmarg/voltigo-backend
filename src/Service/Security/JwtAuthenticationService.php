...
class JwtAuthenticationService
{
    ...
    public function buildWithRolesForUser(User $user, array $rolesAndRights, bool $includeBaseRole = true): string
    {
        ...
        $token = $this->buildTokenForUser($user, [], true);
     ...
     * @throws OptimisticLockException
     */
    public function buildTokenForUser(User $user, array $extraPayload = [], bool $endless = false, bool $saveAsOneTimeUse = true): string
    {
        if ($endless) {
        ...
        $tokenExpirationTimestamp = $this->getTokenExpirationTimestamp($token);

        if ($saveAsOneTimeUse) {
            $oneTimeToken = new OneTimeJwtTokenStorage();
            $oneTimeToken->setToken($token);
            $oneTimeToken->setTokenExpirationTimestamp($tokenExpirationTimestamp);
            $this->oneTimeJwtTokenStorageController->save($oneTimeToken);
        }
    ...
    public function isTokenValid(string $rawToken): bool
    {
        try{
            ...
            $this->loggerService->error("Could not parse the jwt token", [
    ...
    private function refreshJwtToken(string $tokenRaw): string
    {
        try{
            ...
            $user       = $this->userRepository->find($userId, LockMode::NONE); // force fetch fresh entity due to websocket
